...
int find_hash(const char *b)
{
    unsigned char hash[SHA_DIGEST_LENGTH];
    SHA1(b, strlen(b)-1, hash);
    ...
    strcpy(blocks[num_block+1] , hash);
    num_block++;
    return -1;
}
         ...
         struct fuse_file_info *fi)
{
    ...
    for(int i=0 ; i<=(size/4096) ; i++){
        ...
        int ret = find_hash(tmp_buf);
        if(ret == -1)
        {
            // open a block in blockstore and get fd
            strcpy(filename,"/tmp/blockstore/");
            sprintf(blocknum, "%d", num_block);
            strcat(filename,blocknum);
            bb_open(filename,ff);
            log_syscall("pwrite", pwrite(ff->fh, tmp_buf, 4096, 0), 0);
        }
        else{
